...
class PurchaseOrder extends Quickbooks implements QBResourceContract
{
    ...
    {
        $this->service = new \QuickBooks_IPP_Service_PurchaseOrder();
        $query = $this->service->query($this->context, $this->realm, "SELECT * FROM PurchaseOrder WHERE Id = '$id' ")[0];
        if (isset($query)) {
            return $query;
        }
