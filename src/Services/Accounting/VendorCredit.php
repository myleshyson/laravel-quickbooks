...
class VendorCredit extends Quickbooks implements QBResourceContract
{
    ...
    public function update($id, array $data)
    {
        ...
        $this->service->update($this->context, $this->realm, $id, $this->resource) ?: $this->service->lastError();
    }
    ...
    {
        $this->service = new \QuickBooks_IPP_Service_VendorCredit();
        $query = $this->service->query($this->context, $this->realm, "SELECT * FROM VendorCredit WHERE Id = '$id' ")[0];
        if (isset($query)) {
            return $query;
        }
